import { WSEvent } from "../types/websocket.js";
...
export class MeetingSocketHandler {
    ...
    audioChunkBuffers = new Map();
    // Mock transcript interval references when DEEPGRAM_API_KEY is absent
    mockIntervals = new Map();
    constructor(connectionManager, meetingManager, meetingService, transcriptProcessor) {
    ...
    handleConnection(ws, req) {
        ...
        ws.on("message", (rawMessage, isBinary) => {
            ...
            if (isBinary) {
                this.handleAudioChunk(ws, rawMessage);
                return;
            }
    ...
    startDeepgramTranscription(ws, meetingId, participant) {
        ...
        dgConnection.on(LiveTranscriptionEvents.Transcript, (data) => {
            ...
            if (data.is_final) {
                ...
                if (data.speech_final) {
                    ...
                    if (buffer.trim()) {
                        const finalLine = this.transcriptProcessor.formatFinal(participant.id, participant.name, buffer);
                    ...
                    if (buffer.trim()) {
                        const interimSeg = this.transcriptProcessor.formatInterim(participant.id, participant.name, buffer);
                        this.connectionManager.broadcastToRoom(meetingId, {
                ...
                if (currentGuess.trim()) {
                    const interimSeg = this.transcriptProcessor.formatInterim(participant.id, participant.name, currentGuess);
                    this.connectionManager.broadcastToRoom(meetingId, {
                        type: WSEvent.TRANSCRIPT_INTERIM,
                        segment: interimSeg,
                    });
    ...
    startMockTranscriptLoop(ws, meetingId, participant) {
        const phrases = [
